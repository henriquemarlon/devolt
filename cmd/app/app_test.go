...
func (s *AppSuite) TestItCreateStationWithInvalidData() {
	sender := common.HexToAddress("0x0142f501EE21f4446009C3505c51d0043feC5c68")
	input := []byte(`{"path":"createStation","payload":{"owner":"","consumption":-100,"price_per_credit":-50,"latitude":91.0000,"longitude":181.0000}}`)
...
func (s *AppSuite) TestItUpdateStation() {
	...
	input := []byte(`{"path":"updateStation","payload":{"id":1, "owner": "0x1234567890abcdef1234567890abcdef12345678", "consumption": 150, "price_per_credit": 75, "state": "active", "latitude": 34.0522, "longitude": -118.2437}}`)
	expectedOutput = fmt.Sprintf(`updated station - {"id":1,"consumption":"150","owner":"0x1234567890AbcdEF1234567890aBcdef12345678","price_per_credit":"75","state":"active","latitude":34.0522,"longitude":-118.2437,"created_at":%d,"updated_at":%d}`, time.Now().Unix(), time.Now().Unix())
	result = s.tester.Advance(admin, input)
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
}
...
func (s *AppSuite) TestItUpdateStationWithoutPermissions() {
	...
	input := []byte(`{"path":"updateStation","payload":{"id":1, "owner": "0x1234567890abcdef1234567890abcdef12345678", "consumption": 150, "price_per_credit": 75, "latitude": 34.0522, "longitude": -118.2437}}`)
	expectedOutput := `failed to find user by address 0x1234567890AbcdEF1234567890aBcdef12345678: record not found`
	result := s.tester.Advance(sender, input)
	s.ErrorContains(result.Err, expectedOutput)
}
...
func (s *AppSuite) TestItUpdateNonExistentStation() {
	sender := common.HexToAddress("0x0142f501EE21f4446009C3505c51d0043feC5c68")
	input := []byte(`{"path":"updateStation","payload":{"id":10, "owner": "0x1234567890abcdef1234567890abcdef12345678", "consumption": 150, "price_per_credit": 75, "latitude": 34.0522, "longitude": -118.2437}}`)
...
func (s *AppSuite) TestItCreateOrder() {
	...
	expectedOutput = fmt.Sprintf(`created order - {"id":1,"buyer":"0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65","credits":"200","station_id":1,"station_owner":"0x70997970C51812dc3A010C7d01b50e0d17dc79C8","price_per_credit":"50","created_at":%d}`, time.Now().Unix())
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
}
...
func (s *AppSuite) TestItCreateAuctionAndFinishAuctionWithoutPartialSelling() {
	...
	expectedOutput = fmt.Sprintf(`created order - {"id":1,"buyer":"0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65","credits":"200","station_id":1,"station_owner":"%v","price_per_credit":"50","created_at":%d}`, stationOwner01, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
	...
	expectedOutput = fmt.Sprintf(`created order - {"id":2,"buyer":"0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65","credits":"10","station_id":2,"station_owner":"%v","price_per_credit":"10","created_at":%d}`, stationOwner02, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
	...
	expectedOutput = fmt.Sprintf(`created order - {"id":3,"buyer":"0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65","credits":"1000","station_id":1,"station_owner":"%v","price_per_credit":"50","created_at":%d}`, stationOwner01, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	createOrderInput = []byte(`{"path":"createOrder","payload":{"station_id":2}}`)
	result = s.tester.DepositERC20(common.HexToAddress("0x0000000000000000000000000000000000000002"), sender, big.NewInt(100), createOrderInput)
	expectedOutput = fmt.Sprintf(`created order - {"id":4,"buyer":"0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65","credits":"10","station_id":2,"station_owner":"%v","price_per_credit":"10","created_at":%d}`, stationOwner02, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
	...
	expectedOutput = fmt.Sprintf(`created order - {"id":5,"buyer":"0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65","credits":"400","station_id":1,"station_owner":"%v","price_per_credit":"50","created_at":%d}`, stationOwner01, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	createAuctionInput := []byte(fmt.Sprintf(`{"path":"createAuction","payload":{"price_limit_per_credit":"1000","expires_at":%d,"orders_time_range":%d}}`, time.Now().Add(5*time.Second).Unix(), 2))
	result = s.tester.Advance(admin, createAuctionInput)
	expectedOutput = fmt.Sprintf(`created auction - {"id":1,"required_credits":"1620","price_limit_per_credit":"1000","state":"ongoing","expires_at":%d,"created_at":%d}`, time.Now().Add(5*time.Second).Unix(), time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	createBidInput := []byte(`{"path": "createBid", "payload": {"price_per_credit":"100"}}`)
	...
	expectedOutput = fmt.Sprintf(`created bid - {"id":1,"auction_id":1,"bidder":"0x0000000000000000000000000000000000000003","credits":"600","price_per_credit":"100","state":"pending","created_at":%d}`, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	createBidInput = []byte(`{"path": "createBid", "payload": {"price_per_credit":"500"}}`)
	...
	expectedOutput = fmt.Sprintf(`created bid - {"id":2,"auction_id":1,"bidder":"0x0000000000000000000000000000000000000004","credits":"520","price_per_credit":"500","state":"pending","created_at":%d}`, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	createBidInput = []byte(`{"path": "createBid", "payload": {"price_per_credit":"200"}}`)
	...
	expectedOutput = fmt.Sprintf(`created bid - {"id":3,"auction_id":1,"bidder":"0x0000000000000000000000000000000000000005","credits":"200","price_per_credit":"200","state":"pending","created_at":%d}`, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	createBidInput = []byte(`{"path": "createBid", "payload": {"price_per_credit":"300"}}`)
	...
	expectedOutput = fmt.Sprintf(`created bid - {"id":4,"auction_id":1,"bidder":"0x0000000000000000000000000000000000000006","credits":"300","price_per_credit":"300","state":"pending","created_at":%d}`, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
	...
	expectedOutput = `finished auction with - id: 1, required credits: 1620 and price limit per credit: 1000`
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	offSetStationConsumptionInput := []byte(`{"path":"offSetStationConsumption", "payload": {"id": 1, "credits_to_be_offset": 1600}}`)
	result = s.tester.Advance(stationOwner01, offSetStationConsumptionInput)
	expectedOutput = fmt.Sprintf(`offSet Credits from station: 1 by msg_sender: %v`, stationOwner01)
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))

	offSetStationConsumptionInput = []byte(`{"path":"offSetStationConsumption", "payload": {"id": 2, "credits_to_be_offset": 20}}`)
	result = s.tester.Advance(stationOwner02, offSetStationConsumptionInput)
	expectedOutput = fmt.Sprintf(`offSet Credits from station: 2 by msg_sender: %v`, stationOwner02)
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
}
...
func (s *AppSuite) TestItCreateAuctionAndFinishAuctionWithPartialSelling() {
	...
	expectedOutput = fmt.Sprintf(`created bid - {"id":3,"auction_id":1,"bidder":"0x0000000000000000000000000000000000000005","credits":"150","price_per_credit":"200","state":"pending","created_at":%d}`, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
	...
	expectedOutput = fmt.Sprintf(`created bid - {"id":5,"auction_id":1,"bidder":"0x0000000000000000000000000000000000000007","credits":"150","price_per_credit":"200","state":"pending","created_at":%d}`, time.Now().Unix())
	s.Len(result.Notices, 1)
	s.Equal(expectedOutput, string(result.Notices[0].Payload))
