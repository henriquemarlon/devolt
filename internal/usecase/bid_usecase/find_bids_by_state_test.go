...
func TestFindBidsByStateUseCase(t *testing.T) {
	...
	findBidsByState := NewFindBidsByStateUseCase(mockBidRepo)
	...
	mockBidRepo.On("FindBidsByState", uint(1), "pending").Return(mockBids, nil)
	...
	output, err := findBidsByState.Execute(input)
	assert.Nil(t, err)
	assert.NotNil(t, output)
	assert.Len(t, output, 2)

	expectedOutput := FindBidsByStateOutputDTO{
		{
			Id:        1,
			AuctionId: 1,
			Bidder:    common.HexToAddress("0x0"),
